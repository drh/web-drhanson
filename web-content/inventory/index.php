<html>
  ...
  // connect
  $db = DB::connect('mysql://drh:323-Hpo@db70c.pair.com/drh_inventory');
  ...
  // build and issue the query
	$sql = 'SELECT items.id,name,description,
			group_concat(labels.label separator " ") as labels,itemmap.quantity,
			locations.house,locations.room,manufacturer,model,sn,retailer,
			purchased,price,url,dimensions,artist,url,notes
	  FROM items, itemmap, locations, labelmap, labels
	  WHERE items.id=itemmap.itemId and itemmap.roomId=locations.id and
			items.id=labelmap.itemId and labelmap.labelId=labels.id';
	$vals = array();
	if (!empty($q_house)) {
